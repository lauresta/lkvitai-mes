...
public sealed class LayoutEditorClient
{
    ...
    private readonly ILogger<LayoutEditorClient>? _logger;

    public LayoutEditorClient(IHttpClientFactory factory, ILogger<LayoutEditorClient>? logger = null)
    {
        _factory = factory;
        _logger = logger;
    }

    public Task<WarehouseLayoutDto> GetLayoutAsync(string warehouseCode = "Main", CancellationToken cancellationToken = default)
    {
        var encodedWarehouseCode = Uri.EscapeDataString(warehouseCode);
        return GetAsync<WarehouseLayoutDto>($"/api/warehouse/v1/layout?warehouseCode={encodedWarehouseCode}", cancellationToken);
    }
    ...
    private async Task EnsureSuccessAsync(HttpResponseMessage response)
    {
        ...
        _logger?.LogError(
            "Layout editor API request failed. Method={Method} Uri={Uri} StatusCode={StatusCode} ErrorCode={ErrorCode} TraceId={TraceId} Detail={Detail}",
