...
public sealed class DisasterRecoveryClient
{
    ...
    private readonly ILogger<DisasterRecoveryClient>? _logger;

    public DisasterRecoveryClient(IHttpClientFactory factory, ILogger<DisasterRecoveryClient>? logger = null)
    {
        _factory = factory;
        _logger = logger;
    }

    public Task<DRDrillDto> TriggerDrillAsync(string scenario, CancellationToken cancellationToken = default)
        => PostAsync<DRDrillDto>(
            "/api/warehouse/v1/admin/dr/drill",
            new TriggerDrillRequestDto
            {
                Scenario = scenario
            },
            cancellationToken);
    ...
    public Task<IReadOnlyList<DRDrillDto>> GetHistoryAsync(CancellationToken cancellationToken = default)
        => GetAsync<IReadOnlyList<DRDrillDto>>("/api/warehouse/v1/admin/dr/drills", cancellationToken);
    ...
    private async Task EnsureSuccessAsync(HttpResponseMessage response)
    {
        ...
        _logger?.LogError(
            "Disaster recovery request failed. Method={Method} Uri={Uri} StatusCode={StatusCode} ErrorCode={ErrorCode} TraceId={TraceId} Detail={Detail}",
